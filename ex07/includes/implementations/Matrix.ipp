...
{
    if (this->_data.size() <= 0 || vec.size() <= 0)
        throw std::runtime_error("Could not compute mul_vec: Matrix or Vector is empty");
    if (this->_data[0].size() != vec.size())
        throw std::runtime_error("Could not compute mul_vec: Matrix and vector are not the same size.");
    ...
    for (size_t i = 0; i < this->_rows; i++)
    {
        float   temp = 0;
        for (size_t j = 0; j < this->_cols; j++)
            temp = std::fma(this->_data[i][j], vec[j], temp);
        res.push_back(temp);
    }
    return Vector<K>(res);
}
...
{
    if (this->_rows <= 0 || mat.getRows() <= 0)
        throw std::runtime_error("Could not compute mul_mat: Matrix is empty");
    if (this->_cols != mat.getRows())
        throw std::runtime_error("Could not compute mul_mat: Matrix are not the same size.");

    std::vector<std::vector<K>> res(this->_rows, std::vector<K>(mat.getCols(), 0));
    for (size_t i = 0; i < this->_rows; i++)
    {
        for (size_t j = 0; j < mat.getCols(); j++)
        {
            ...
            for (size_t k = 0; k < this->_cols; k++)
                temp = std::fma(this->_data[i][k], mat[k][j], temp);
            res[i][j] = temp;
        }        
    }
    return Matrix<K>(res);
