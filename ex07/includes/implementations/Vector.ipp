...
{
    if (u.size() != v.size())
        throw std::runtime_error("Could not compute angle_cos: Vectors not the same size.");
    if (u.size() == 0)
        throw std::runtime_error("Could not compute angle_cos: Vector empty.");
    
    float res = (u.dot(v)) / (u.norm() * v.norm());
...
Vector<K>   cross_product(const Vector<K> &u, const Vector<K> &v)
{
    if (u.size() != v.size() || u.size() != 3)
        throw std::runtime_error("Could not compute cross_product: Vectors not the correct size.");
    
    std::vector<K>  res;
    res.push_back(std::fma(u[1], v[2], -(u[2]*v[1])));
    res.push_back(std::fma(u[2], v[0], -(u[0]*v[2])));
    res.push_back(std::fma(u[0], v[1], -(u[1]*v[0])));
    return Vector<K>(res);
}